...
exports.listAdmin = async (req, res, next) => { try { const data = await Announcement.find().populate('createdBy', 'email').populate('user', 'email name').sort({ createdAt: -1 }).limit(100).lean(); res.json({ success: true, data }); } catch (error) { next(error); } };
